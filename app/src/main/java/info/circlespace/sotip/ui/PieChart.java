...
public class PieChart extends ViewGroup {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mPieBounds.offsetTo(w/3f, getPaddingTop());

        mPointerX = mPieBounds.centerX();
        mPointerY = mPieBounds.bottom - h/12f;
    ...
    private void onDataChanged() {
        ...
        for (int i=0; i<mData.size(); i++) {
            ...
            it.mEndAngle =  currentAngle + Math.round((it.mValue / mTotal ) * 360.0f );

            // to account for imprecise calculations
            if ( i == mData.size() - 1) {
                if ( it.mEndAngle < 360 )
                    it.mEndAngle = 360;
            }
    ...
    private void init() {
        ...
        // Set up the paint for the pointer
    ...
    private class PointerView extends View {
        ...
        @Override
        protected void onDraw(Canvas canvas) {
            canvas.drawLine(mPointerX, mPointerY, mPointerX, canvas.getHeight(), mTextPaint);
